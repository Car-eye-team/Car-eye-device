...
public class CameraSurfaceView extends SurfaceView implements SurfaceHolder.Callback, PreviewCallback, CameraGLSurfaceView.OnRenderListener {
		...
		try {
			...
			if (mCamera != null) {
				...
				if (mGLSurfaceView != null) {
					...
					//注释后变成全屏预览,如果长宽比和屏幕一样而不加1，就会导致预览小图不见了。。。
					mGLSurfaceView.setAspectRatio(ScreenUtils.getScreenWidth(), ScreenUtils.getScreenHeight()- BarUtils.getNavBarHeight()+1);
	...
	public void onPreviewFrame(byte[] data, Camera camera) {
		...
		if (mGLSurfaceView != null) {
			...
			if (imageData != null) {
				byte[] buffer = imageData.getData();
				System.arraycopy(data, 0, buffer, 0, buffer.length);
				if (mOnCameraListener != null) {
					imageData.setParams(mOnCameraListener.onPreview(buffer, mWidth, mHeight, mFormat, timestamp));
//					imageData.mParams = mOnCameraListener.onPreview(buffer, mWidth, mHeight, mFormat, timestamp);
	...
	public void onBeforeRender(CameraFrameData data) {
		if (mOnCameraListener != null) {
//			((CameraFrameData2)data).mTimeStamp = System.nanoTime();
